...
impl BrowserTool {
    ...
    fn call_obscura_tool(&self, tool_name: &str, args: serde_json::Value) -> anyhow::Result<String> {
        let bin = obscura_path();
        if !bin.exists() {
            bail!("obscura binary not found at {}. Run: bash scripts/setup.sh", bin.display());
        }

        // Use our existing MCP client to talk to obscura
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(async {
            let mut client = crate::mcp::client::McpClient::new(
                bin.to_str().unwrap(),
                &["mcp"],
            )?;
...
impl ToolHandler for BrowserTool {
    fn schema(&self) -> ToolSchema {
        ToolSchema {
            name: "browser".into(),
            description: "obscura CDP headless browser — navigate, screenshot, click, type, evaluate JS"
                .into(),
...
impl BrowserTool {
    fn op_navigate(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        ...
        if let Ok(mut u) = self.current_url.lock() { *u = final_url.clone(); }
        if let Ok(mut h) = self.current_html.lock() { *h = body.clone(); }
        ...
        Ok(format!(
            "## Navigate\n\nURL: {}\nStatus: {}\n\n```html\n{}{}\n```\n\n**{} bytes**",
            final_url, status.as_u16(), preview, suffix, body.len()
        ))
    }

    fn op_get_html(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        let selector_str = args.get("selector").and_then(|v| v.as_str()).unwrap_or("body");
        let html = self.current_html.lock().map_err(|e| anyhow!("lock: {}", e))?;
        if html.is_empty() { bail!("no page loaded — call navigate first"); }
...
impl BrowserTool {
    fn op_screenshot(&self, _args: &serde_json::Value) -> anyhow::Result<String> {
        match self.call_obscura_tool("browser_screenshot", serde_json::json!({})) {
            Ok(result) => Ok(result),
            Err(e) => Ok(format!("## Screenshot\n\n⚠ obscura unavailable: {}\n\nRun: bash scripts/setup.sh", e)),
        }
    }
    ...
    fn op_type_text(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        ...
        match self.call_obscura_tool("browser_type", serde_json::json!({"selector": selector, "text": text})) {
            Ok(r) => Ok(r),
            Err(e) => Ok(format!("## Type\n\nSelector: {}\nText: {}\n\n⚠ obscura unavailable: {}", selector, text, e)),
        }
    }

    fn op_evaluate(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        let js = args.get("javascript").and_then(|v| v.as_str()).unwrap_or("");
